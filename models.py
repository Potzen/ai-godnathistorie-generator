...
class Story(db.Model):
    ...
    series_part = db.Column(db.Integer,
                            default=1)  # Sporer progression. En ny historie er Del 1. En fortsættelse er Del 2, osv.
    parent_story_id = db.Column(db.Integer, db.ForeignKey('story.id'),
                                nullable=True)  # Selvrefererende nøgle, der skaber en kæde af fortsættelser.
    ...
    children_stories = db.relationship('Story', backref=db.backref('parent_story', remote_side=[id]),
                                       lazy='dynamic')  # Gør det nemt at finde alle efterfølgere.
    ...
    child_values = db.Column(db.Text,
                             nullable=True)  # De underliggende værdier, barnet forsvarede (f.eks. "Mod, Venskab").
