...
def generate_story():
    ...
    # 1. Udpak alle nødvendige data fra request først
    ...
    is_interactive = data.get('interactive', False)  # Hent is_interactive her
    is_bedtime_story = data.get('is_bedtime_story', False)
    negative_prompt_text = data.get('negative_prompt', '').strip()
    selected_model_from_frontend = data.get('selected_model')  # Hentes også her

    current_app.logger.info(
        f"--- generate_story route: is_interactive flag from frontend: {is_interactive} ---")

    # 2. Bearbejd data til prompt-strenge (karakter_str, sted_str, plot_str)
    ...
    if listeners:
        ...
        # Betinget afslutnings-instruktion baseret på is_bedtime_story
        ending_instruction = "VIGTIGT OM AFSLUTNINGEN: Afslut historien på en positiv og opløftende måde."  # Standard for Højtlæsning

        if is_bedtime_story:
            # Hvis det ER en godnathistorie, tilføjes den specifikke "Sov godt"-afslutning
            if names_list_for_ending:
                ...
                # Fallback for en godnathistorie, hvor der ikke er angivet navne
    ...
    # 3. Bestem AI-model, længde instruktion, max_tokens_setting og safety_settings
    # (Dette er den kodeblok, der var placeret forkert eller manglede i dit udsnit)
    target_model_name = 'gemini-1.5-flash-latest'
    if current_user.is_authenticated and current_user.role in ['basic', 'premium']:
        default_pro_model = 'gemini-2.5-pro-preview-05-06'
        if selected_model_from_frontend == default_pro_model:
            target_model_name = default_pro_model
        elif selected_model_from_frontend == 'gemini-1.5-flash-latest':
            target_model_name = 'gemini-1.5-flash-latest'
        else:  # Hvis intet eller ugyldigt sendt, og bruger er basic/premium, brug pro
            target_model_name = default_pro_model

    current_app.logger.info(
        f"Valgt AI-model for historiegenerering: {target_model_name} (Brugerrolle: {current_user.role if current_user.is_authenticated else 'Gæst'}, Frontend valg: {selected_model_from_frontend})")
    ...
    safety_settings = {}  # Default til tom dict, hvis ingen specifikke settings er nødvendige
    if target_model_name == pro_model_identifier:
        current_app.logger.info(f"Anvender lempelige safety settings (BLOCK_NONE) for pro-model: {target_model_name}")
        safety_settings = {
    ...
    # 4. Definer generation_config_dict (NU er max_tokens_setting defineret)
    generation_config_dict = {
        "max_output_tokens": max_tokens_setting,
        "temperature": 0.7
    }

    # 5. Byg prompten (NU er alle nødvendige strenge og instruktioner defineret)
    prompt = build_story_prompt(
        ...
        is_bedtime_story=is_bedtime_story  # NY PARAMETER
    )

    # 6. Log prompten (som du allerede har)
    if is_interactive:
        current_app.logger.debug(
            f"--- generate_story route (INTERAKTIV): Fuld prompt bygget:\n{prompt}\n---")
    else:
        current_app.logger.debug(
            f"--- generate_story route (STANDARD): Prompt bygget (første 500 tegn):\n{prompt[:500]}\n---")
    ...
    try:
        story_title, actual_story_content = generate_story_text_from_gemini(
            ...
            safety_settings_values=safety_settings,  # Send de korrekte safety_settings
